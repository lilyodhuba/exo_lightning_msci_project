...
## Updated using data from Sergeev et al. (2025) MNRAS 541, 3155-3166
def temp_strat(
    sim_params: SimulationParameters,
    pressure: float, 
)-> float:
    initial_pressure = sim_params.start_pressure
    log10sigma_p = np.log10(pressure / initial_pressure)
...
def run_sim(sim_params: SimulationParameters, const: PhysicalConstants) -> dict:
    ...
    for i in range(stepmax):
        ...
        if (np.log10(P / sim_params.start_pressure) < -1.6) or (Tfallnew < 181):
